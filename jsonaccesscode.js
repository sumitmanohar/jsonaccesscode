...
var info = function(name, weaknesses) {
    //str is variable of json data
    for (i = 0; i < str.pokemon.length; i++) {
        if (str.pokemon[i].name == name) {
            console.log(str.pokemon[i])
            //check next_evolution is present or not
            if ('next_evolution' in str.pokemon[i]) {
                //if next_evolution is present then find out lenght of that next_evolution
                for (j = 0; j < str.pokemon[i].next_evolution.length; j++) {
                    console.log("pokemon name in next_evolution :" + str.pokemon[i].next_evolution[j].name)
                }
            } else {
                console.log("next_evolution field is not present ")
            }
        }
    }
    for (i = 0; i < str.pokemon.length; i++) {
        //find length of pokemon weaknesses
        for (j = 0; j < str.pokemon[i].weaknesses.length; j++) {
            //check input weaknesses
            if (str.pokemon[i].weaknesses[j] == weaknesses) {
                console.log("pokemon that have same weaknesses :" + str.pokemon[i].name)
                break
            }
        }
    }
}
//enter the name of pokemon of given data
var name = window.prompt("enter the name")
//enter the weaknesses of that pokemon
var weaknesses = window.prompt("enter the weaknesses")
info(name, weaknesses)
